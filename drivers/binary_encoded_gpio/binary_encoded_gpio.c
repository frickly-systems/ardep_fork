...
#define BINARY_ENCODED_GPIO_PIN_COUNT 3

struct binary_encoded_gpio_config {
  struct gpio_dt_spec input_pins[BINARY_ENCODED_GPIO_PIN_COUNT];
};
...
static int get_value(const struct device *device) {
  ...
  for (int i = 0; i < BINARY_ENCODED_GPIO_PIN_COUNT; i++) {
...
#define BINARY_ENCODED_GPIO_INIT(inst)                                      \
  static const struct binary_encoded_gpio_config                            \
      binary_encoded_gpio_config_##inst = {                                 \
        .input_pins =                                                       \
            {                                                               \
              DT_INST_FOREACH_PROP_ELEM_SEP(inst, input_gpios,              \
                                            GPIO_DT_SPEC_GET_BY_IDX, (, )), \
            },                                                              \
  };                                                                        \
  BUILD_ASSERT(                                                             \
      DT_INST_PROP_LEN(inst, input_gpios) == BINARY_ENCODED_GPIO_PIN_COUNT, \
      "Binary encoded GPIO driver requires exactly " STRINGIFY(BINARY_ENCODED_GPIO_PIN_COUNT) " input_gpios");                   \
